...
const DynamicForm = ({ gridMeta, idform, record, onClose, allGrids, readonlyMode }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isUpdate) {
                // 1. Campo marcado como pk en metadata
                ...
                // 2. Convención Ghenesis: id + nombre de tabla (ej: idform para xforms)
                if (!pkFieldName) {
                    const tableName = (gridMeta.vquery || '').replace(/^x/, '').replace(/s$/, '');
                    const candidates = [
                        `id${gridMeta.vquery}`,  // idxforms
                        `id${tableName}`,         // idform
                    ];
                    pkFieldName = candidates.find(c => record[c] !== undefined);
                }
                // 3. Primer campo que empiece con 'id'
                if (!pkFieldName) {
